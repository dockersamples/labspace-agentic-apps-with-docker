...
1. In the :tabLink[Visual Chatbot]{id="chatbot" href="http://localhost:3050"}, clear the messages by clicking the **Reset messages** button at the bottom of the thread.

2. Update the system prompt to have the following prompt:

    ```plaintext
    You are a helpful agent that is happy and polite, yet concise. You answer questions, yet don't add too much fluff. But, be sure to answer the question completely and thoroughly.

    When tool execution is used, assume the user does not see the result of the tool execution. Be sure to use any results from tool executions in your response.
    ```
...
2. Add a new user message asking for the current time.
    ...
    The result you either get a non-answer back or it will be wildly inaccurate. That's because the model doesn't have access to the current date and time. Let's change that!
...
1. In the :tabLink[Visual Chatbot]{href="http://localhost:3050" id="chatbot"}, reset all of the messages.
...
7. Once you have a tool execution result, click the **Send messages to model** button to get a response from the model.
